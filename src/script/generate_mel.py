...
if __name__ == '__main__':
    ...
    if os.path.isfile(teacher_utt_path):
        ...
        print(f"ac_mel: {ac_mel}")
        with open(os.path.join(output_dir, 'test_mel.npy'), 'wb') as f:
           mel = np.squeeze(ac_mel.cpu().detach().numpy()).transpose(1, 0)
           np.save(f, mel)
           print(f"mel file shape: {mel.shape}")
        #output_file = os.path.join(output_dir, 'test_mel')
    else:
